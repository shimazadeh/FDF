...
int	draw(t_data *data)
{
	...
	while (y < data->length)
	{
		...
		while (x < data->width)
		{
			img_pix_put(&data->img, data->array[y][x].x_screen,
				data->array[y][x].y_screen, data->array[y][x].color);
			if (y != data->length - 1)
				drawline(&data->array[y][x], &data->array[y + 1][x], data->img);
			if (x != data->width - 1)
				drawline(&data->array[y][x], &data->array[y][x + 1], data->img);
			x++;
		}
		y++;
	}
...
{
	if (end->x_screen < start->x_screen)
		execute(end, start, img);
	else
		execute(start, end, img);
	return ;
}
